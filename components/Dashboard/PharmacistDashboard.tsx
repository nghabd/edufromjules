								...
								filteredCourses.map((course) => {
									...
									return (
										<CourseCard
											...
											expanded={expandedCourses.has(course.assignmentId)}
											onToggle={() =>
												setExpandedCourses((current) => {
													const next = new Set(current);
													if (next.has(course.assignmentId)) next.delete(course.assignmentId);
													else next.add(course.assignmentId);
													return next;
												})
											}
											meta={[
												{
													label: "lessons",
													value: `${course.completedTopics}/${course.totalTopics}`,
												},
												...(course.totalParts > course.totalTopics
													? [
															{
																label: "parts",
																value: `${course.completedParts}/${course.totalParts}`,
															},
														]
													: []),
												...(course.dueDate
													? [
															{
																label: isCourseOverdue
																	? "overdue"
																	: `due ${format(new Date(course.dueDate), "MMM d")}`,
																value: isCourseOverdue ? (
