// lib/course-payload-write.ts
//
// Write-path helpers for building Prisma create/update data when a course is
// authored or edited. This file intentionally imports `sanitizeHTML`, which
// pulls in the jsdom/dompurify dependency chain — that's fine here, because
// this file is only ever imported by routes that actually create or update
// course content (e.g. the course builder save endpoint), not by read-only
// routes like /api/admin/overview.
//
// Do NOT import this file from a route that only reads/lists courses — use
// `@/lib/course-payload-read` for that instead, to avoid pulling in the
// sanitizer's dependency chain unnecessarily.
...
// Re-exported here for convenience so any file that already imports from
// course-payload-write for the write helpers can also get the include shape
// without a second import. The canonical source of truth for this constant
// is course-payload-read.ts.
export { courseResponseInclude } from "@/lib/course-payload-read";
